    ...
    try {
      const feedback = await this.prisma.feedback.create({
        data: {
          ...
          appVersion: dto.appVersion?.trim() || '2.2.8',
          nps: Number(dto.nps),
      ...
      try {
        ...
        `, protocol, resolvedName, resolvedName, dto.email?.trim() || null, dto.device?.trim() || 'Desconhecido', dto.androidVersion?.trim() || null, dto.appVersion?.trim() || '2.2.8', Number(dto.nps), dto.problema?.trim() || 'nenhum', dto.descricao?.trim() || null, dto.screenshotDesc?.trim() || null);
        ...
        try {
          const result: any = await this.prisma.$queryRawUnsafe(`
            ...
            RETURNING "id", "protocol"
          `, protocol, protocol, resolvedName, resolvedName, dto.email?.trim() || null, dto.device?.trim() || 'Desconhecido', dto.androidVersion?.trim() || null, dto.appVersion?.trim() || '2.2.8', Number(dto.nps), dto.problema?.trim() || 'nenhum', dto.descricao?.trim() || null, dto.screenshotDesc?.trim() || null);
