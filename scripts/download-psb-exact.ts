...
async function main() {
  const info = await getJson(
    'https://commons.wikimedia.org/w/api.php?action=query&titles=File:Partido_Socialista_Brasileiro_logo.svg&prop=imageinfo&iiprop=url&iiurlwidth=500&format=json'
  );
  ...
  if (thumbUrl) {
    ...
    if (buf) {
      ...
      console.log(`✅ Saved official party_psb.png (${buf.length} bytes)`);
    }
  }
}
