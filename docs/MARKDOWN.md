...
- **Principais módulos:** Auth, Prioridades (Stateless), Matching, Candidate Management, Geo, ETL, CI/CD, Cola Eleitoral, Ads (Anúncios Éticos), Finance (Sustentabilidade PIX), Public-API (Tiered API), Watchdog (Observatório de Mandatos), Reports (Relatórios B2B)
...
| Doc | Descrição |
|-----|-----------|
| [PRD.md](./PRD.md) | Product Requirements Document — User Stories, 13 pilares, busca nacional com rolagem lateral de cargos, Cola Eleitoral 2026, blindagem cibernética e diretrizes de interesse nacional |
...
| [DOCUMENTACAO_TECNICA.md](./DOCUMENTACAO_TECNICA.md) | Arquitetura, schemas, APIs, auditoria TSE, ColaModule (PDFKit), algoritmo de matching, módulos de monetização (ads, finance, public-api, watchdog, reports) e blindagem defense-in-depth |
...
| [MANUAL_DO_USUARIO.md](./MANUAL_DO_USUARIO.md) | Fluxo de uso do app, busca nacional com rolagem lateral de cargos (wheel/touch/botões), fotos oficiais, interpretação da justificativa e Cola Eleitoral (PDF/WhatsApp) |
...
| [DATA_SOURCES.md](./DATA_SOURCES.md) | Fontes de dados públicos (Portal de Dados Abertos do TSE, Câmara, Senado) |
| [PRIVACY_FIRST.md](./PRIVACY_FIRST.md) | Arquitetura de privacidade (Privacy by Design, cola 100% local e transparência eleitoral) |
...
│   │   │   ├── (auth)/          # Login, Cadastro
│   │   │   ├── (tabs)/          # Candidatos (busca nacional), cola (Minha Cola Eleitoral 2026), matching (Consulta por Prioridades), Perfil, raio-x
│   │   │   ├── candidate/[id]   # Raio-X detalhado (card expandido auditável)
│   │   │   ├── transparencia.tsx# /transparencia: Livro-caixa aberto, custos de infra e arrecadação PIX
│   │   │   ├── observatorio.tsx # /observatorio: Monitoramento de votações nominais e promessômetro
│   │   │   ├── relatorios.tsx   # /relatorios: Catálogo cívico B2B com telemetria agregada Coleta Zero
│   │   │   ├── anuncie.tsx      # /anuncie: Onboarding self-service de anunciantes éticos
